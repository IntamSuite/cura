...
from PyQt5.QtGui import QOpenGLTexture, QImage, QTransform
    ...
    def bind(self, texture_unit):
        if not self._qt_texture.isCreated():
            if self._file_name != None:
                angle = QTransform()
                ##angle.setMatrix(0, 0, 1, 0, 1, 0, -1, 0, 0)
                self._image = QImage(self._file_name).mirrored()
                ##self._image = QImage(self._image).transformed(angle.rotate(90,axis = 0))
